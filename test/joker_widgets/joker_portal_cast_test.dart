...
void main() {
  ...
  testWidgets('JokerPortal and JokerCast work with Presenter',
      (WidgetTester tester) async {
    // Arrange
    final counterPresenter = TestPresenter<int>(5, tag: 'pCounter');
    final namePresenter = TestPresenter<String>('Presenting', tag: 'pName');

    int counterBuildCount = 0;
    int nameBuildCount = 0;

    // Act - Initial build using Presenter instances in JokerPortal
    await tester.pumpWidget(
      JokerPortal<int>(
        // Use base type for portal if needed, but specific type for Cast
        joker: counterPresenter, // Provide Presenter instance
        tag: 'pCounter',
        child: JokerPortal<String>(
          joker: namePresenter, // Provide Presenter instance
          tag: 'pName',
          child: Column(
            children: [
              JokerCast<int>(
                // Cast consumes the int state
                tag: 'pCounter',
                builder: (context, value) {
                  counterBuildCount++;
                  return Text('PCount($counterBuildCount): $value',
                      textDirection: TextDirection.ltr);
                },
              ),
              JokerCast<String>(
                // Cast consumes the String state
                tag: 'pName',
                builder: (context, value) {
                  nameBuildCount++;
                  return Text('PName($nameBuildCount): $value',
                      textDirection: TextDirection.ltr);
                },
              ),
            ],
    ...
    // Assert - Initial state & lifecycle
    expect(counterBuildCount, 1);
    expect(nameBuildCount, 1);
    expect(find.text('PCount(1): 5'), findsOneWidget);
    expect(find.text('PName(1): Presenting'), findsOneWidget);
    expect(counterPresenter.initCalled, isTrue);
    expect(namePresenter.initCalled, isTrue);
    await tester.pump(); // For onReady
    expect(counterPresenter.readyCalled, isTrue);
    expect(namePresenter.readyCalled, isTrue);

    // Act - Change only counter presenter state
    counterPresenter.updateState(10);
    await tester.pump();

    // Assert - Only counter rebuilt
    expect(counterBuildCount, 2);
    expect(nameBuildCount, 1);
    expect(find.text('PCount(2): 10'), findsOneWidget);
    expect(find.text('PName(1): Presenting'), findsOneWidget);

    // Act - Change only name presenter state
    namePresenter.updateState('Done');
    await tester.pump();
    ...
    expect(nameBuildCount, 2);
    expect(find.text('PCount(2): 10'), findsOneWidget);
    expect(find.text('PName(2): Done'), findsOneWidget);

    // Remove widget to check disposal
    await tester.pumpWidget(Container());
    await tester.pump();
    expect(counterPresenter.doneCalled, isTrue);
    expect(counterPresenter.isDisposed, isTrue);
    expect(namePresenter.doneCalled, isTrue);
    expect(namePresenter.isDisposed, isTrue);
  });
}
